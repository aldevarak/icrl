...
<div id="paginar">
    ...
    $sql_des="SELECT * FROM tg013_productos WHERE in_estatus='1' ".$sql_stock."";
	...
	while ($producto = mysqli_fetch_array($resultados_des)) {
		...
		//
			...
			if (file_exists($nombre_fichero)) {
				...
				$pro_des.="<div class='photo2' style=\"background-image: url('".$nombre_fichero_new."');\"></div>";
			}else{
				$pro_des.="
						<div class='photo'></div>";
			}
			...
			if($producto['nu_stock']<=0){
				...
				if (($opc_precio=="1")||($opc_precio=="2")){
					$pro_des.="<p class='btn-add hidden-xs'><i class='fa fa-shopping-cart'></i><a class='hidden-xs' href='javaScript:;' onclick=\"parent.cambio('../fichas/carrito-total.php?id=".$producto['co_productos']."&modo=agregar',	'cont');\">Agregar</a></p>
